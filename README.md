...
跨平台算法刷题数据看板，保留 GitHub 风格年度热力图、提交流、标签统计和错题本；后端已切换为 Go，数据同步改为**手动同步**。

## 现在的同步方式

CodeActivityHub 不执行后台定时轮询。历史数据、公开用户资料、题库和比赛，在你点击“刷新看板”、某个平台的「同步记录」或打开对应页面时，由 Go 后端按需调用各平台公开接口（或你填的 Cookie）读取。提交记录同样通过「同步记录」按钮手动同步，不会在后台自动抓取。

看板在页面切换或点击“刷新看板”时读取最新数据，可随时手动刷新查看。

平台凭证只保存在本地数据库，只用于以你的身份向对应平台读取提交记录，不会发送给其他站点。
...
scripts/dev-build-frontend.sh        # 前端本地构建辅助脚本
...
登录 CodeActivityHub 后，进入 **系统设置 → 脚本接入**，点「生成新 Token」并立即复制弹出的明文
（明文只显示一次，库里只存哈希）。Token 是**独立于登录会话的长效凭证**：不会 7 天过期，
可用于提交记录接入接口（其他接口返回 403），可在设置页按设备逐条轮换或吊销，改密码时自动全部吊销。
脚本已移除，接口保留供后续接入使用。
...
- **同步**：取消后台定时轮询；历史数据与提交记录均由用户主动点击后通过公开接口手动同步。
- **用户体验**：保留看板、热力图、标签和错题本；设置页新增长效 Token 管理，轮询配置标记为停用。
